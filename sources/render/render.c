...
/*   Updated: 2026/04/09 13:11:03 by thlibers         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	weapon(t_game *game)
{
    ...
    while (y < game->w_height)
    {
        ...
        {
            game->weapon.dst = game->weapon_addr + (y * game->weapon_line_length) + (x * (game->weapon_bits_per_pixel / 8));
            game->weapon.color = *(int *)game->weapon.dst;
            if (game->weapon.color != 0x000000)
                my_mlx_pixel_put(game, game->weapon.start_x + x, game->weapon.start_y + y, game->weapon.color);
            x++;
        }
        y++;
    }
